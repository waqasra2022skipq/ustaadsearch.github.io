...
### Added
- "Recommended teachers" now has a "Download CV" option alongside Email invite/WhatsApp/Copy/Profile, both on the institution dashboard's job detail panel and the matching Filament admin widget — `cv_path` was already present (signed S3 URL) in the shared `recommendedTeacherPayload()` response consumed by both surfaces, so this was UI-only: no backend or type changes needed (frontend + Filament)
